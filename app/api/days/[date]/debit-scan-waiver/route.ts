...
/** PUT { waived: boolean, note?: string } — calendar-day "missing/misprinted debit slip" without a scan file. */
export async function PUT(request: NextRequest, { params }: { params: Promise<{ date: string }> }) {
  try {
    ...
    const row = await prisma.debitScanDayWaiver.upsert({
      where: { date },
      create: { date, note },
      update: { note }
    })
    ...
    console.error('debit-scan-waiver PUT', e)
