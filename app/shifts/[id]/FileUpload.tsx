...
export default function FileUpload({ shiftId, type, currentUrls, onUploadComplete }: FileUploadProps) {
  ...
  const handleUpload = async () => {
    ...
    try {
      const uploadPromises = filePreviews.map(async (filePreview) => {
        ...
        const res = await fetch(`/api/shifts/${shiftId}/upload`, {
          method: 'POST',
          body: formData
        })
  ...
  const handleRemove = async (urlToRemove: string) => {
    ...
    try {
      const updateData = type === 'deposit' 
        ? { depositScanUrls: JSON.stringify(newUrls) }
        : { debitScanUrls: JSON.stringify(newUrls) }
