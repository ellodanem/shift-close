...
export type WeeklySundayTiming = 'not_due' | 'due' | 'overdue'

/** Weekly task due on week-ending Sunday; overdue from Monday. Optional 6 AM grace on Sunday. */
export function weeklySundayTimingStatus(
  asOfYmd: string,
  weekSundayYmd: string,
  now: Date = new Date()
): WeeklySundayTiming {
  if (compareYmd(asOfYmd, weekSundayYmd) < 0) return 'not_due'
  if (compareYmd(asOfYmd, weekSundayYmd) === 0) {
    if (isBeforeShiftDueGrace(now, weekSundayYmd)) return 'not_due'
    return 'due'
  }
  return 'overdue'
}

export function calendarMonthFromYmd(ymd: string): { year: number; month: number } {
  const [year, month] = ymd.split('-').map(Number)
  return { year, month }
}

export function monthQueryFromParts(year: number, month: number): string {
  return `${year}-${String(month).padStart(2, '0')}`
}

/** Mon-start week keys from epoch week through current week (inclusive). */
export function enumerateWeekKeysFromEpoch(asOf: string, epochYmd: string): string[] {
  const start = weekKeyMonday(epochYmd)
  const end = weekKeyMonday(asOf)
  if (compareYmd(start, end) > 0) return []
  const keys: string[] = []
  let cur = start
  while (compareYmd(cur, end) <= 0) {
